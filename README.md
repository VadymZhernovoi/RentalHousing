...
# Только просмотры
python manage.py seed_stats --searches=0

# Только поиски + пересчёт
python manage.py seed_stats --no-views --searches=500

# Только пересчитать агрегаты без генерации
python manage.py seed_stats --no-views --no-searches
```
